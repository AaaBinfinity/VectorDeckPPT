...
每套模板位于独立目录，包含一张总览图和 10 张逐页生成的 16:9 SVG/PNG 页面：封面、章节、叙事、问题、流程、证据、对比、路线图、决策和结尾。当前 12 套、共 120 张正式页面已全部重构；不是只强化 Dynamic Hero，也不是给同一套卡片骨架换颜色。每个页面角色都有独立的几何轮廓和信息任务，普通内容页同时展示结论、解释、证据/例子和影响/行动。艺术字体只用于 `deck-title` / `section-title` 等展示角色，普通页标题、同级标题和正文仍锁定为整套一致的精确字体 token，并通过严格字体审计。

> **库级质量门禁：** 每套必须有 10 个可区分的页面轮廓；12 套叙事与构图签名不得退化成彼此的换色副本；全部 120 张 SVG 必须通过验证和严格字体一致性审计。
...
<table>
  ...
  <tr>
    ...
    <td width="33%" align="center">
      ...
      <br><sub>多层英雄字 · 斜切舞台 · 网点与速度线 · 原创信号徽记</sub>
    </td>
  </tr>
  <tr>
    <td width="33%" align="center">
...
> **模板负责启发，不负责替代判断。** 选择模板时先看总览，再打开该目录的 `slides/` 检查逐页效果；评价时要看标题舞台、前后景、反形、裁切、方向力、微观节奏和信息密度，不能只看配色。示例数字只用于展示信息层级，必须替换成真实来源。专业场合默认采用克制、可信、证据优先的视觉方向，但专业不等于所有页面都横平竖直：可以使用编辑式不对称、受控叠压、裁切、弧线、斜线和少量英雄字体建立艺术感。书法或展示字体只用于封面、章节和少数关键结论，正文不使用艺术字。
