...
| `polygon`/`polyline` | PNG preview + Office SVG relation | Fallback | object editable; points not native |
| gradient/clip/rotated element | PNG preview + Office SVG relation | Fallback | object-level editing only |
| forbidden/invalid element | compilation error | Failed | no output deck |

`freeform` remains reserved in the report for future native polygon/path work and is currently normally `0`.
...
- `freeform`: future native freeform conversions;
...
- Complex path-to-PowerPoint freeform conversion is not implemented.
