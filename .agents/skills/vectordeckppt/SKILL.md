...
4. Produce the complete text-only slide draft before authoring SVG. Present it to the user and save `pptoutput/slide-content.md`. For every slide include number, takeaway title, purpose, key message, substantive audience-facing copy, supporting points or explanation, evidence/source, and a concrete chart/diagram/image plan. Default to information-rich content rather than sparse topic labels.
...
7. Select exactly three representative slides for visual prototyping: the opening, a representative information-rich core-content page, and the most visually demanding evidence/diagram page. Ensure the sample demonstrates the default text density and at least one meaningful chart or diagram when sources permit it. For decks shorter than three pages, select every page. State which pages were selected and why.
