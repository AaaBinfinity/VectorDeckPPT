...
- When a user supplies a visual reference because a bundled family feels plain, revise the family as a complete ten-page system rather than polishing only its overview or cover. Preserve the reference's abstract behaviors—dominant mass, typographic staging, counter-shapes, crop tension, depth, micro-rhythm, and density—while rebuilding every page role with original graphics and useful sample content.
- Reject built-in template pages that are only a heading plus a few generic cards. Every bundled family must provide ten role-specific page silhouettes—cover through close—and the twelve family decks must not collapse into one shared geometry with palette swaps. Make the narrative, context, process, evidence, comparison, roadmap, and decision pages substantial enough to judge real deck behavior.
- When a user asks to improve every template, treat all twelve bundled families and all 120 page pairs as the scope. Regenerate and audit the whole library; do not present a single expressive family as evidence that the library was fixed.
