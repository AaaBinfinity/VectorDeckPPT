...
如果用户已经给出某项信息，不重复询问该项。必须补齐并确认：主要受众身份、知识水平和决策权，次要受众，沟通目标，演示场景与正式程度，资料与事实边界，语言、页数、比例和时长，视觉方向、视觉来源与禁用项，品牌素材，编辑性、交付物和输出位置。视觉来源必须是用户选择的内置方向、上传参考、明确批准的混合方向，或用户明确委托 Agent 代选。
...
禁止把“蓝紫渐变 + 大量圆角卡片 + 无意义图标”当成通用科技美学。可以选择十二套内置视觉参考之一，也可以上传或定义新的方向；参考图只用于提取字体、空间、色彩、图像和构图行为，不能直接贴成整页背景。
...
通用图标
少量主题参考
十二套按独立目录管理的 16:9 风格参考；每套包含总览与 10 张逐页 SVG/PNG
测试素材
```
