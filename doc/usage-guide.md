...
| 视觉来源 | 选择内置方向、上传 PPT/PPTX/PDF/图片/截图/品牌参考，或委托 Agent 代选 | 避免 Agent 默认套用自己偏好 |
...
Skill 会用少量分组问题补齐缺失信息，再提交一份简明合同摘要。只有用户明确确认受众、目标、场景、资料、事实边界、页数、语言、视觉方向、视觉来源、品牌约束、交付物和默认项后，才开始资料综合与逐页规划。后续发现实质矛盾时会返回该确认门。

### 如何选择或上传模板

- 没有参考文件时，Agent 根据受众、场景、证据类型和情绪强度只推荐 3 套，分别说明优势和取舍；
- 每套内置模板都在 `assets/style-templates/<family>/` 下独立管理，`overview.png` 用于快速选择，`slides/` 包含 10 页逐页 SVG/PNG 参考；
- 选择前不只看总览：至少检查封面、一页信息丰富的内容页和一页证据/图解页；
- 可上传现有 PPT/PPTX、PDF、截图、图片或品牌规范作为视觉来源；Agent 不只匹配颜色，还会提取主视觉质量、标题舞台、反形、裁切、叠压、方向力、微观节奏、景深和密度曲线，不直接复制水印、受保护角色、Logo 或未授权素材；
- 如果用户指出某套内置方向“太简约”或“只像卡片”，应重做该方向的完整 10 页视觉系统和生成器，而不是只替换总览图或给封面加装饰；如果用户明确说“每个模板都要改”，范围就是 12 套、120 张逐页 SVG/PNG，不能只展示 Dynamic Hero 或另一套强视觉方向；
- 用户可以选一套、批准最多两套语法混合，或明确说“由 Agent 选择”。不明确时，Agent 不得默认套用某一风格。
...
设计样稿前先锁定字体角色表。所有普通页面标题共享同一个精确字号、字体、字重和行高；同一页中承担相同层级的标题、标签或数字使用同一个 token。书法、手写、压缩或其他艺术字体作为独立的 `deck-title` / `section-title` 角色，主要用于封面、章节和少数关键结论；正文和普通页标题使用可读、可交付的字体。

文字溢出时应改写、换行、扩大区域或调整构图，不单独缩小一个标题。每个可见 SVG `<text>` 都要有语义 `data-role`，全套页面在编译前必须通过严格字体审计。

内置模板库还要做库级审计：每套十页必须有十个可区分的页面轮廓，十二套的几何与叙事签名不能只是换色副本；正式资产生成后再对全部 120 张 SVG 运行验证与严格字体审计。
